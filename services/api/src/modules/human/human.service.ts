...
export class HumanService {
  ...
  async findNearbyResponders(
    ...
    this.logger.log(
      `Searching for responders within ${radiusMeters}m of ${lat}, ${lng}`,
    );
    ...
    return [
      {
        userId: 'resp-1',
        fullName: 'Verified Responder Yaba',
        distance: '450m',
        trustScore: 88.5,
      },
      {
        userId: 'resp-2',
        fullName: 'Tejuosho Medical Node',
        distance: '1.2km',
        trustScore: 95.0,
      },
    ];
  ...
  async assignResponder(
    ...
    this.logger.log(
      `Assigning responder ${responderId} to incident ${incidentId}`,
    );
  }
}
